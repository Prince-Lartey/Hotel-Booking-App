...
const BookingForm = ({ currentUser, paymentIntent }: Props) => {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-5">
            ...
            <label className="text-gray-700 text-sm font-bold flex-1">
                Email
                <input className="mt-1 border rounded w-full py-2 px-3 text-gray-700 bg-gray-200 font-normal" type="email" readOnly disabled {...register("email")}/>
            </label>
            ...
            <div className="flex justify-end">
                <button disabled={isLoading} type="submit" className="bg-slate-500 text-white p-2 font-bold hover:bg-slate-400 text-md disabled:bg-slate-400">{isLoading ? "Saving..." : "Confirm Booking"}</button>
            </div>
        </form>
    )
}
